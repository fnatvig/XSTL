...
def import_data(X_pretrain, X_train, first):
    ...
    if first: 
        ...
        return df_pretrain, df_train


def preprocess_data(df_pretrain, df_train, df_test, wnd_size, first):
...
def tee_print(message, file):
    ...
    print(message, file=file)  # File

def test_hypothesis(fpr_a, fpr_b, auc_a, auc_b, test, ablation, log_file):
    arr_a = np.array(fpr_a)
    arr_b = np.array(fpr_b)
    
    # Constructing ID for baseline (given the ID of test)
    baseline = ""
    freezing = ""
    if ablation:
        letter = ""
    ...
    # Writing results to both stdout and results.txt
    tee_print("\n", log_file)
    tee_print(f"___Results_from_{test}_FPR@TPR=1.0___", log_file)
    stat_sign = ""
    if (result1.pvalue<0.05) or (result2.pvalue<0.05):
        stat_sign = "(STATISTICALLY SIGNIFICANT)"
    if ablation:
        tee_print(f"{freezing} FPR = {np.mean(arr_a)} +- {np.std(arr_a)}", log_file)
        tee_print(f"{test} FPR = {np.mean(arr_b)} +- {np.std(arr_b)}", log_file)
    else:
        tee_print(f"{test} FPR = {np.mean(arr_a)} +- {np.std(arr_a)}", log_file)
        tee_print(f"{baseline} FPR = {np.mean(arr_b)} +- {np.std(arr_b)}", log_file)
    if result2.pvalue<result1.pvalue:
        if ablation:
            tee_print(f"{test} (FPR) p_value = {result2.pvalue} (l) - Freezing ({freezing}) is better {stat_sign}", log_file)
        else:
            tee_print(f"{test} (FPR) p_value = {result2.pvalue} (l) - XSTL ({test}) is better {stat_sign}", log_file)
    else:
        if ablation:
            tee_print(f"{freezing} (FPR) p_value = {result1.pvalue} (r) - Non-freezing ({test}) is better {stat_sign}", log_file)
        else:
            tee_print(f"{freezing} (FPR) p_value = {result1.pvalue} (r) - Baseline ({baseline}) is better {stat_sign}", log_file)
    ...
    if (result1.pvalue<0.05) or (result2.pvalue<0.05):
        stat_sign = "(STATISTICALLY SIGNIFICANT)"
    if ablation:
        tee_print(f"{freezing} AUC = {np.mean(arr_a)} +- {np.std(arr_a)}", log_file)
        tee_print(f"{test} AUC = {np.mean(arr_b)} +- {np.std(arr_b)}", log_file)
    else:
        tee_print(f"{test} AUC = {np.mean(arr_a)} +- {np.std(arr_a)}", log_file)
        tee_print(f"{baseline} AUC = {np.mean(arr_b)} +- {np.std(arr_b)}", log_file)
    if result2.pvalue<result1.pvalue:
        if ablation:
            tee_print(f"{test} (AUC) p_value = {result2.pvalue} (l) - Non-freezing ({test}) is better {stat_sign}", log_file)
        else:
            tee_print(f"{test} (AUC) p_value = {result2.pvalue} (l) - Baseline ({baseline}) is better {stat_sign}", log_file)
    else:
        if ablation:
            tee_print(f"{freezing} (AUC) p_value = {result1.pvalue} (r) - Freezing ({freezing}) is better {stat_sign}", log_file)
        else:
            tee_print(f"{freezing} (AUC) p_value = {result1.pvalue} (r) - XSTL ({test}) is better {stat_sign}", log_file)
...
def main(log_file):
    ...
    for id in range(len(tests)):
        test = tests[id]

        print(f"Running Test: {test}")
        X_pretrain, X_train, ablation = parse_test(test)

        print("Reading data. This could take a while...")
        if id % 3 == 0:
            df_pretrain, df_train, df_test_raw = import_data(X_pretrain, X_train, first=True)
        else: 
            df_pretrain, df_train= import_data(X_pretrain, X_train, first=False)

        print("Preprocessing data. This will take even longer...")
        if id % 3 == 0:
            df_pretrain, df_train, df_test = preprocess_data(df_pretrain, df_train, df_test_raw, wnd_size=2, first=True)
        else:
            df_pretrain, df_train = preprocess_data(df_pretrain, df_train, df_test_raw, wnd_size=2, first=False)
        ...
        test_hypothesis(fpr_A, fpr_B, auc_A, auc_B, test, ablation, log_file)
